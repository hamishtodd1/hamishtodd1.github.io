...
vec4 laplacian()
{
	...
	vec4 uv0 = texture2D( oldState, vUV + vec2( 1./dimensions.x,0.));
	vec4 uv1 = texture2D( oldState, vUV + vec2(-1./dimensions.x,0.));
	vec4 uv2 = texture2D( oldState, vUV + vec2( 0., 1./dimensions.z/dimensions.y) );
	vec4 uv3 = texture2D( oldState, vUV + vec2( 0.,-1./dimensions.z/dimensions.y) );
	vec4 uv4 = texture2D( oldState, vUV + vec2( 0., 1./dimensions.z) );
	vec4 uv5 = texture2D( oldState, vUV + vec2( 0.,-1./dimensions.z) );
...
void main (void)
{
	...
	float unew = u + laplacian().r * 0.1; // this works so maybe try reducing h
	float vnew = v;
	// float unew = u < 0.5 ? 0.:1.;
	// float vnew = v;
