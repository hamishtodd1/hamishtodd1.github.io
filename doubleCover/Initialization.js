...
function initDoubleSphere()
{
	...
	doubleSphere.triangles = Array(4*6*2);
	doubleSphere.triangles[0] = new THREE.Mesh(new THREE.Geometry(),
			new THREE.MeshPhongMaterial({color:0xFF0000} ) );
	doubleSphere.add(doubleSphere.triangles[0]);
	
	//tetrakis hexahedron. Could have triakis octahedron
	doubleSphere.triangles[0].geometry.vertices.push(new THREE.Vector3( 0,0,1 ) );
	doubleSphere.triangles[0].geometry.vertices.push(new THREE.Vector3( 1,1,1 ) );
	doubleSphere.triangles[0].geometry.vertices.push(new THREE.Vector3( -1,1,1 ) );
	for(var i = 0; i < doubleSphere.triangles[0].geometry.vertices.length; i++)
		doubleSphere.triangles[0].geometry.vertices[i].setLength(0.1);
	doubleSphere.triangles[0].geometry.faces.push( new THREE.Face3( 0,1,2 ) );
	doubleSphere.triangles[0].geometry.computeFaceNormals();
	doubleSphere.triangles[0].geometry.computeVertexNormals();
	
	for(var i = 1; i < doubleSphere.triangles.length; i++)
	{
		doubleSphere.triangles[i] = doubleSphere.triangles[0] = new THREE.Mesh( doubleSphere.triangles[0].geometry,
				doubleSphere.triangles[0].material.clone() );
	...
	doubleSphere.spectatorDirection = new THREE.Vector3();
	
	doubleSphere.add(doubleSphere.surface);
...
function initRP2()
{
	...
	RP2.spectatorDirection = new THREE.Vector3();
