...
async function initOneQubit() {
...

    {
        ...
        updateFunctions.push(() => {
            let dest = inside ? camera.position.z : camera.position.z / 3.
...

    {
        ...
        bindButton("q", () => {
            ...
            initialMouseMv.meet(e0, e0MeetInitialMouse)
            mvToJoinTo.copy(e12)
        }, null, () => {
            if (!(mouseMv.equals(initialMouseMv)))
                mvToJoinTo.copy(mouseMv)
            e0MeetInitialMouse.join( mvToJoinTo, torusLine)
        },false,()=>{
            torusLine.mul(stateMv,mv1)
            stateMv.copy(mv1)
        ...
        updateFunctions.push(() => {
            ...
            e3.mul( torusLine, torusRotorSquared)
