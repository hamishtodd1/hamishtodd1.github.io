...
function mergeAttributes( attributes ) {
	...
	for ( let i = 0; i < attributes.length; ++ i ) {
		...
		if ( TypedArray !== attribute.array.constructor ) {
			...
			console.error( 'THREE.BufferGeometryUtils: .mergeAttributes() failed. THREE.BufferAttribute.array must be of consistent array types across matching attributes.' );
			return null;
		...
		if ( itemSize !== attribute.itemSize ) {
			...
			console.error( 'THREE.BufferGeometryUtils: .mergeAttributes() failed. THREE.BufferAttribute.itemSize must be consistent across matching attributes.' );
			return null;
		...
		if ( normalized !== attribute.normalized ) {
			...
			console.error( 'THREE.BufferGeometryUtils: .mergeAttributes() failed. THREE.BufferAttribute.normalized must be consistent across matching attributes.' );
			return null;
		...
		if ( gpuType !== attribute.gpuType ) {
			...
			console.error( 'THREE.BufferGeometryUtils: .mergeAttributes() failed. THREE.BufferAttribute.gpuType must be consistent across matching attributes.' );
			return null;
...
function toCreasedNormals( geometry, creaseAngle = Math.PI / 3 /* 60 degrees */ ) {
	...
	// THREE.BufferGeometry.toNonIndexed() warns if the geometry is non-indexed
