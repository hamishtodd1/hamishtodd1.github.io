...
function initUserDefinedField(){
    ...
    let voxelsWide = 32 // it doesn't like more than 128. 128 is fine but slow to load
    ...
    putSphereInField = (center) => {
        for (let row = 0; row < dimensions.y; ++row) {
            for (let column = 0; column < dimensions.x; ++column) {
                ...
                let val = .5 - v1.distanceTo(center)
                if (val < 0.)
                    val = 0.

                let firstIndex = (row * dimensions.x + column) * 4
                stateArray[firstIndex + 0] += val //Math.random()
                stateArray[firstIndex + 1] += 0.
                stateArray[firstIndex + 2] += 0.
