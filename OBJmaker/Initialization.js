...
function init_bocavirus_stuff()
{
	...
	OurOBJLoader.load( "http://hamishtodd1.github.io/Data/stmvProtein.obj",
		function ( masterProtein ) 
		{
//			for(var i = 0; i < )
//			masterProtein.scale.setScalar(0.001);
			
			console.log(masterProtein)
			...
			var stmvProteins = Array(20);
			
			for(var i = 0; i < stmvProteins.length; i++)
			{
				stmvProteins[i] = new THREE.Mesh( masterProtein.children[0].geometry.clone(), new THREE.MeshPhongMaterial());
				Protein.add(stmvProteins[i]);
			}
			
			function rotate_protein_bunch(ouraxis, amt, seed_index)
			{
				stmvProteins[seed_index].worldToLocal(ouraxis);
				ouraxis.normalize();
				for(var i = 0; i < 12; i++)
				{
					stmvProteins[i*5+seed_index].rotateOnAxis(ouraxis, amt);
					stmvProteins[i*5+seed_index].updateMatrixWorld();
				}
			}
			...
			for(var j = 0; j < 60; j += 15)
			{
				for(var i = 5; i < 10; i++ )
				{
					...
					stmvProteins[j+i].worldToLocal(specific_da);
					stmvProteins[j+i].rotateOnAxis(specific_da, TAU / 3);
					stmvProteins[j+i].updateMatrixWorld();
				}
				...
				for(var i =10; i < 15; i++ )
				{
					...
					specific_da.normalize();
					stmvProteins[j+i].worldToLocal(specific_da);
					stmvProteins[j+i].rotateOnAxis(specific_da, 2 * TAU / 3);
					stmvProteins[j+i].updateMatrixWorld();
				}
			}
			...
			for(var i = 15; i < 30; i++)
			{
				...
				stmvProteins[i].worldToLocal(specific_da);
				stmvProteins[i].rotateOnAxis(specific_da, TAU / 2);
				stmvProteins[i].updateMatrixWorld();
			}
			for(var i = 30; i < 45; i++)
			{
		...
		function ( xhr ) {}, function ( xhr ) { console.error( "couldn't load stmv" ); } );
}
