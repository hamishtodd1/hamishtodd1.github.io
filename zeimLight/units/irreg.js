...
//			ico.updateMatrixWorld();
			...
			{
				var clientLine = new THREE.Line3( mouse.rayCaster.ray.origin, mouse.rayIntersectionWithZPlane(0) )
				grabbableEdges[i].parent.worldToLocal(clientLine.start);
				grabbableEdges[i].parent.worldToLocal(clientLine.end);
				
				//it lies in the plane orthogonal to the edge and in the z = 0 plane. Could also set from normal
				var pointInEdgePlane = grabbableEdges[i].line3.delta(new THREE.Vector3()).cross(clientLine.start).add(grabbableEdges[i].start);
				
				var edgePlane = new THREE.Plane().setFromCoplanarPoints(grabbableEdges[i].start, grabbableEdges[i].end, pointInEdgePlane);
				
				var edgePlaneClientRayIntersection = edgePlane.intersectLine( clientLine, new THREE.Vector3() );
				if( edgePlaneClientRayIntersection )
				{
					...
					if( 0 < closestPointOnEdgeLineParameter && closestPointOnEdgeLineParameter < 1 &&
							edgeRadius * 3 > grabbableEdges[i].line3.at( closestPointOnEdgeLineParameter, new THREE.Vector3() ).distanceTo( edgePlaneClientRayIntersection )
					)
					{
		...
		if( mouse.clicking )
		{
			if( !mouse.oldClicking )
			{
				if( hoveringEdge !== -1)
				{
					...
					//get triangle that we'll see as flat
					{
						...
						var edgeInIQ = grabbableEdges[ grabbedEdge ].line3.delta(new THREE.Vector3()).applyQuaternion(ico.quaternion).setZ(0).normalize();
						var edgeInDQ = grabbableEdges[ grabbedEdge ].line3.delta(new THREE.Vector3()).applyQuaternion(destinationQuaternion).setZ(0).normalize();
			...
			{
				var clientLine = new THREE.Line3( mouse.rayCaster.ray.origin, mouse.rayIntersectionWithZPlane(0) )
				grabbableEdges[grabbedEdge].parent.worldToLocal(clientLine.start);
				grabbableEdges[grabbedEdge].parent.worldToLocal(clientLine.end);
				...
				var facePlaneClientRayIntersection = facePlane.intersectLine( clientLine, new THREE.Vector3() );
