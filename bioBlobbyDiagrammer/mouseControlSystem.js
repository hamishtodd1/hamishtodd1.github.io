...
function initMouse() 
{
	...
	mouse.updateFromAsyncAndCheckClicks = function()
	{
		...
		mouse.rayCaster.ray.origin.x = camera.left + (camera.right-camera.left) * asynchronous.normalizedDevicePosition.x
		mouse.rayCaster.ray.origin.y = camera.bottom + (camera.top-camera.bottom) * asynchronous.normalizedDevicePosition.y
		...
		mouse.position.copy(this.rayIntersectionWithZPlane(0))
