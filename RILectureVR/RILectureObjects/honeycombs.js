...
//				|| i+j+k > 0 
	...
	atomicCubicLattice.update = function()
	{
		...
		for(var i = 0, il = latticeShadow.children.length; i < il; i++ )
		{
			...
			latticeShadow.children[i].position.z = 0;
			//speedup opportunity: single geometry
		}
		latticeShadow.position.copy(atomicCubicLattice.position);
		latticeShadow.position.z = -0.195;
	}
	
	
	
	var ourloader = new THREE.OBJLoader();
	torch = presentation.createNewHoldable("torch");
	ourloader.load(
		'http://hamishtodd1.github.io/RILectureVR/Data/Light_01.obj',
		function ( torchOBJ ) {
			torchOBJ.position.z = 1.5;
			torchOBJ.scale.setScalar(0.02);
			torchOBJ.rotation.set( -3.080830943850726,-0.09013410513963895,2.7846474082321517);
